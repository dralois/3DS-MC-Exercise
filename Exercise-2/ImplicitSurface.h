...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		return pow(_x.x() - m_center.x(), 2.0) + pow(_x.y() - m_center.y(), 2.0) + pow(_x.z() - m_center.z(), 2.0) - pow(m_radius, 2.0);
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		return pow(pow(_x.x() - m_center.x(), 2.0) + pow(_x.y() - m_center.y(), 2.0) + pow(_x.z() - m_center.z(), 2.0) + pow(m_radius, 2.0) - pow(m_a, 2.0), 2.0) -
			(4.0 * pow(m_radius, 2.0) * (pow(_x.x() - m_center.x(), 2.0) + pow(_x.y() - m_center.y(), 2.0)));
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		for (uint32_t i = 0; i < m_numCenters; ++i)
		{
			result += m_coefficents[i] * EvalBasis((m_funcSamp.m_pos[i] - _x).norm());
		}

		result += m_coefficents.segment(m_numCenters, 3).dot(_x);
...
#define phi(i,j) EvalBasis((m_funcSamp.m_pos[i]-m_funcSamp.m_pos[j]).norm())
	...
	void BuildSystem()
	{
		...
		A.block(0, m_numCenters + 3, 2 * m_numCenters, 1).setOnes();

		for(uint32_t y = 0; y < 2 * m_numCenters; ++y)
		{
			for(uint32_t x = 0; x < m_numCenters; ++x)
			{
				A(y, x) = phi(y, x);
			}
			A.block<1, 3>(y, m_numCenters) = m_funcSamp.m_pos[y];
			b(y) = m_funcSamp.m_val[y];
		}
